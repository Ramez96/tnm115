...
function routing_score(res, jsonString) {
    ...
    if (susScore == null) sendResponse(res, 406, null, null);        // send response with status code 406 (Not Acceptable), according to server documentation (see start of the file)
        ...
        storeSusResultsinDB(susJsonDataFromClient.results, susScore)
            .catch(console.error)
            .finally(() => {
                dbClient.close();
...
async function storeSusResultsinDB(resultJsonArray, finalScore) {

    const dbSusDocument = {
        timestamp: new Date().toISOString(),
        system_name: "TNM115-Lessons-Interface",
        sus_score: finalScore,
        item_results: resultJsonArray
    };

    await dbClient.connect();
    const db = dbClient.db("tnm115-sus");
    const dbCollection = db.collection("sus-submissions");
    const insertResult = await dbCollection.insertOne(dbSusDocument);
    console.log("Inserted SUS Result Document:", insertResult);
